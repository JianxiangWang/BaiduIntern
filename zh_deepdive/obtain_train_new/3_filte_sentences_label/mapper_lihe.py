...
def main():
    ...
    for line in sys.stdin:
        ...
        for P in line["mars_ner"]:
            ...
            # to mention_list
            # s_list = [("刘德华", "PER", 0, 3), ...] == > [(mention_id, mention_text, sent_id, begin_index, end_index, "刘德华", "PER", 0, 3), ...]
            s_mention_list = []
            for s in s_list:
            ...
            for s_mention in s_mention_list:
                for o_mention in o_mention_list:
                    ...
                    for label, rule, info in label_for_current_P(
                            tokens,
                            s_mention[1], s_mention[3], s_mention[4],
                            o_mention[1], o_mention[3], o_mention[4],
                            P
                    ):
                        wanted = True
                        ...
                        so_s = "%s|_|%s|_|%d|_|%d" % (s_mention[-4], s_mention[-3], s_mention[-2], s_mention[-1])
                        so_o = "%s|_|%s|_|%d|_|%d" % (o_mention[-4], o_mention[-3], o_mention[-2], o_mention[-1])
        ...
        if wanted:
            ...
            depparser = json.dumps(line["depparser"], ensure_ascii=False)

            sent_sent = " ".join(new_tokens)

            print "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
                sent_id, sent_text, tokens, pos_tags, ner_tags, dep_types, dep_tokens, S_O, depparser,
...
# s: ("刘德华", "PER", 0, 3)
# return (mention_id, mention_text, sent_id, begin_index, end_index, "刘德华", "PER", 0, 3)
def _get_mention(sent_text, sent_id, tokens, s):
    ...
    if begin_index and end_index:
        ...
        if set(unicode(mention_text_)) == set(unicode(so_mention_text)) :
            return (mention_id, mention_text, sent_id, begin_index, end_index, s[0], s[1], s[2], s[3])
